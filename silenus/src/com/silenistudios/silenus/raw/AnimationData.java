...
/** This data structure contains all points and locations for one animation
 * (scene). Can be easily serialized and recovered later for re-use.
 * @author Karel */
public class AnimationData implements Serializable {
	...
	public AnimationData (int animationLength, int width, int height, int frameRate) {
		...
		for (int i = 0; i < fAnimationLength; ++i)
			fFrames[i] = new AnimationFrameData();
	}
