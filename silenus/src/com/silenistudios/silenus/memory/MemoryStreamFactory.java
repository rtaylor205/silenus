...
/** This factory represents a virtual file system that can be used to unzip and
 * parse an FLA file and generate the appropriate subfiles (png's), without
 * having to save them on disk. This stream factory can be used if you want to
 * generate only JSON or raw animation data, without changing anything to the
 * file system. You will first have to create an output stream to copy the
 * original FLA to, so that you can then call set the stream factory on an
 * XFLDocument and call XFLDocument.parse with the correct filename for the
 * output stream you wrote the FLA to. XFLDocument will then work completely
 * within this virtual filesystem.
 * @author Karel */
