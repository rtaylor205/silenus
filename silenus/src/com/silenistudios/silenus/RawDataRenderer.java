...
/** This class will not render the animation to any video output, but will
 * instead "render" the resulting locations of each object at each frame to a
 * data structure that can be saved later for easy reconstruction of the
 * animation. This allows you to send the original .png images along with this
 * data to completely reproduce the animation without having to compute it
 * real-time.
 * @author Karel */
public class RawDataRenderer implements RenderInterface {
	...
	public void rotate (double theta) {
		...
		fTransformationMatrix = TransformationMatrix.compose(fTransformationMatrix, new TransformationMatrix(0.0, 0.0, 1.0, 1.0,
			theta));
	...
	@Override
	public void save () {
		fTransformationStack.add(new TransformationMatrix(fTransformationMatrix.getMatrix(), fTransformationMatrix.getTranslateX(),
			fTransformationMatrix.getTranslateY()));
