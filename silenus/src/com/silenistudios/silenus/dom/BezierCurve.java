...
/** This class generates cubic bezier curves for animation ease. Two methods are
 * supported: simple ease (with an acceleration value) and custom ease (defined
 * by points).
 * @author Karel */
public class BezierCurve {
	...
	public BezierCurve (double[] x, double[] y) throws ParseException {
		...
		if (x.length != y.length)
			throw new ParseException("Failed to load custom ease: invalid x/y data provided");
		if ((x.length - 1) % 3 != 0)
			throw new ParseException("Failed to load custom ease: invalid number of points");
	...
	/** @return the X parameters */
	public double[] getX () {
		return fX;
	}
	
	/** @return the Y parameters */
	public double[] getY () {
		return fY;
	}
