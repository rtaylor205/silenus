...
public class Point {
	...
	private static final Pattern RegExp = Pattern
		.compile("((?:[-]?[0-9]*[\\.]?[0-9]+)|(?:[-]?[#]?[0-9A-F]*[\\.]?[0-9A-F]+))\\s+((?:[-]?[0-9]*[\\.]?[0-9]+)|(?:[-]?[#]?[0-9A-F]*[\\.]?[0-9A-F]+))");
	...
	public Point (String xTwip, String yTwip) {
		...
		// hex or int?
		if (xTwip.startsWith("#"))
			fX = (int)(parseHex(xTwip.substring(1)) * 2);
		else {
			try {
		...
		if (yTwip.startsWith("#"))
			fY = (int)(parseHex(yTwip.substring(1)) * 2);
		else {
			try {
	...
	private double parseHex (String twip) {
		...
		// there is a fraction - make sure it is appended properly
			...
			fullString += split[1];
			if (split[1].length() == 1)
				fullString += "0"; // not sure if this ever happens, but you never know with adobe
		}
