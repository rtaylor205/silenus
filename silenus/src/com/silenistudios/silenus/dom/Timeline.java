...
/** A timeline contains all info regarding transformations of child objects
 * attached to its graphic. A timeline in the DOMDocument.xml file corresponds
 * to a scene in flash.
 * @author Karel */
public class Timeline {
	...
	public Timeline (XMLUtility XMLUtility, XFLLibrary library, Node root) throws ParseException {
		...
		for (int i = 0; i < layers.size(); ++i) {
			...
			if (layer.getMaxFrameIndex() > fMaxFrameIndex) {
				fMaxFrameIndex = layer.getMaxFrameIndex();
			}
	...
	public Set<Bitmap> getUsedImages (Set<String> symbolInstancesAlreadyChecked) {
		...
		for (Layer layer : fLayers) {
			v.addAll(layer.getUsedImages(symbolInstancesAlreadyChecked));
		}
		return v;
	}
