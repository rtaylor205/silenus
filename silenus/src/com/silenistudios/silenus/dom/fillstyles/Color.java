...
/** A simple color object. Doubles as Paint, so that it can "render" itself using
 * the ShapeRenderInterface.
 * @author Karel */
public class Color implements Paint {
	
	private static Pattern PatternHex = Pattern
		.compile("\\#([0-9a-fA-F][0-9a-fA-F])([0-9a-fA-F][0-9a-fA-F])([0-9a-fA-F][0-9a-fA-F])"); // html color code #XXXXXX	
	
	private static Pattern PatternRGB = Pattern.compile("rgb\\(\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*\\)"); // rgb
	
	// regular expressions for parsing a color
	private static Pattern PatternRGBA = Pattern
		.compile("rgba\\(\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]+)\\s*,\\s*([0-9]*\\.?[0-9]+)\\s*\\)"); // rgba
	...
	public static final Color parseColor (String s) {
		...
		if (res.matches() && res.groupCount() == 4)
			return new Color(Integer.parseInt(res.group(1)), Integer.parseInt(res.group(2)), Integer.parseInt(res.group(3)),
				Double.parseDouble(res.group(4)));
		...
		if (res.matches() && res.groupCount() == 3)
			return new Color(Integer.parseInt(res.group(1), 16), Integer.parseInt(res.group(2), 16), Integer.parseInt(res.group(3),
				16));
	...
	public String toString () {
		return new StringBuilder(21).append("rgba(").append(fRed).append(',').append(fGreen).append(',').append(fBlue).append(',')
			.append(fAlpha).append(')').toString();
	}
}
