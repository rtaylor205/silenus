...
/** An IK child node contains a reference ID which refers to a symbol or bitmap
 * instance, and a set of x/y/rotation values for each frame of the animation.
 * Important note: for some reason, the x/y values are stored in the XML with a
 * *20 multiplier, compared to all other x/y values. We divide by 20 here to
 * avoid this strange issue.
 * @author Karel */
public class IKChildNode {
	...
	public IKChildNode (XMLUtility XMLUtility, Node root) throws ParseException {
		...
		// must be same size
		if (xArraySplit.length != yArraySplit.length || yArraySplit.length != angleArraySplit.length)
			throw new ParseException(
				"Failed to parse IK ChildNode: xArray, yArray and angleArray must contain the same number of values");
		...
		for (int i = 0; i < xArraySplit.length; ++i) {
			TransformationMatrix mx = new TransformationMatrix(Double.parseDouble(xArraySplit[i]) / 20.0,
				Double.parseDouble(yArraySplit[i]) / 20.0, 1.0, 1.0, Double.parseDouble(angleArraySplit[i]));
