...
/** This class will parse an entire XFL scene and generate keyframe data for all
 * objects. It will load the different graphics and call the RenderInterface to
...
public class XFLDocument implements XFLLibrary {
	...
	public Bitmap getBitmap (String href) throws ParseException {
		...
		if (bitmap == null)
			throw new ParseException("Bitmap reference could not be resolved: " + href);
	...
	public Graphic getGraphic (String href) throws ParseException {
		...
		// not found - throw exception
		if (graphic == null)
			throw new ParseException("Graphic reference could not be resolved: " + href);
	...
	private void loadInclude (final Map<String, Node> nameToNode, Node node) throws ParseException {
		...
		if (name.equals(""))
			throw new ParseException("Invalid filename found for include: '" + href + "'");
	...
	private void unzipFLA (String fileName, String pathName) throws ParseException {
		try {
			...
			throw new ParseException("Failed to read FLA (zip) file '" + fileName
				+ "'. Perhaps you are uploading a CS4 or older FLA file?", e);
		}
	}
}
